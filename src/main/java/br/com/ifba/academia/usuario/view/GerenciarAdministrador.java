...
public class GerenciarAdministrador extends javax.swing.JFrame {
    ...
    public GerenciarAdministrador(UsuarioIController usuarioService, PessoaIController pessoaService) {
    ...
    private void tblAlunosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblAlunosMouseClicked
        ...
        if (coluna == 3) { 
            try {
                Usuario admin = usuarioService.findById(id);
                CadastroAdministrador telaCadastro = new CadastroAdministrador(usuarioService, pessoaService);
            ...
            int resposta = JOptionPane.showConfirmDialog(this, "Deseja excluir este admin?", "Confirmação", JOptionPane.YES_NO_OPTION);
            ...
            for (Usuario u : lista) {
                if ("ADMIN".equals(u.getPerfil())) {
                    modeloTabela.addRow(new Object[]{
                        u.getId(), u.getPessoa().getNome(), u.getLogin(), "✏️ Editar", "❌ Excluir"
                    });
                }
            }
        } catch (Exception e) {
